...
xlim = [np.min(im.dE1[1,:])/4, np.max(im.dE1[1,:])*2.5]

name = "10n-InSe SI-003"

"""
# Predictions for scaled intensities, plot 1 complete spectrum, plot 2 zoomed in to [0, xlim] eV
...
ax5.set_title("predictions for spectrum pixel[20,20] of " + name)
ax5.set_xlabel("energy loss [eV]")
ax5.set_ylabel("intensity")
ax6.set_title("predictions for spectrum pixel[20,20] of " + name)
...
# Plot single spectrum of pixel
for i in range(1,7):
    fig7, ax7 = plt.subplots()
    fig8, ax8 = plt.subplots()
    ax7.set_title("ZLP matching results at pixel[30," + str(20*i) + "]" + name)
    ax7.set_xlabel("energy loss [eV]")
    ax7.set_ylabel("intensity")
    ax8.set_title("ZLP matching results at pixel[30," + str(20*i) + "]" + name)
    ax8.set_xlabel("energy loss [eV]")
    ax8.set_ylabel("intensity")
    ax8.set_ylim(-500,3e3)
    ax8.set_ylim(-200,1.5e3)
    ax8.set_xlim(xlim)
    
    signal = im.get_pixel_signal(30,20*i)
    
    ZLPs = im.calc_gen_ZLPs(30,20*i)
    low = np.nanpercentile(ZLPs, 16, axis=0)
    high = np.nanpercentile(ZLPs, 84, axis=0)
    mean = np.average(ZLPs, axis = 0)
    mean = np.nanpercentile(ZLPs, 50, axis=0)
    ax7.fill_between(im.deltaE, low, high, alpha = 0.2)
    ax8.fill_between(im.deltaE, low, high, alpha = 0.2)
    ax7.plot(im.deltaE, mean, label = "gen")
    ax8.plot(im.deltaE, mean, label = "gen")
    
    ZLPs = im.calc_ZLPs(30,20*i)
    low = np.nanpercentile(ZLPs, 16, axis=0)
    high = np.nanpercentile(ZLPs, 84, axis=0)
    mean = np.average(ZLPs, axis = 0)
    mean = np.nanpercentile(ZLPs, 50, axis=0)
    ax7.fill_between(im.deltaE, low, high, alpha = 0.2)
    ax8.fill_between(im.deltaE, low, high, alpha = 0.2)
    ax7.plot(im.deltaE, mean, label = "matched")
    ax8.plot(im.deltaE, mean, label = "matched")
    ...
    mean = np.average(ZLPs, axis = 0)
    mean = np.nanpercentile(ZLPs, 50, axis=0)
    ax7.fill_between(im.deltaE, signal-low, signal-high, alpha = 0.2)
    ax8.fill_between(im.deltaE, signal-low, signal-high, alpha = 0.2)
    ax7.plot(im.deltaE, signal-mean, label = "result")
    ax8.plot(im.deltaE, signal-mean, label = "result")

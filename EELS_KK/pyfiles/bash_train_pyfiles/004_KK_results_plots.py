...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 12 22:41:38 2021
...
if plotim == 'lau':
    ...
    save_loc = "../../plots/final_report/lau/lau_"
...
if plotim == '004':
    ...
    save_loc = "../../plots/final_report/004/KK/png/"
    try_pixels = [[7,65],[45,65],[83,65]]
...
ssratio1 = ss_sum_med/ieels_sum
ssratio1_CI = (ss_sum_high-ss_sum_low)/ieels_sum/2
"""
...
im.plot_heatmap(np.maximum(im.t[:,:,0],0), title = "thickness " + name+", capped at max 200", cbar_kws={'label': '[nm]'}, cmap = cmap, vmax=200, save_as = save_loc + "t")
im.plot_heatmap(np.maximum(im.t[:,:,0],0), title = "thickness "+ name +", capped at max 200", cbar_kws={'label': '[nm]'}, vmax = 200, cmap = cmap,  mask=mask, save_as = save_loc + "t_capped_40")
...
im.plot_heatmap((im.E_band[:,:,2]-im.E_band[:,:,1])/im.E_band[:,:,0]/2, title = "relative broadness CI bandgap energies "+ name +", \ncapped at max 0.5", cbar_kws={'label': 'energy [eV]'}, vmax=0.5, cmap = cmap, save_as = save_loc + "E_band_CI_capped", mask = mask)
...
if plotim == 'lau':
    path_to_models = 'models/report/lau_clu10_p5_final_5dE1_06dE1/'
    path_to_results = "../../KK_results/report/image_KK_lau_clu10_pooled_5_5dE1_06dE1_5iter_dE1_0_set25.pkl"
    # path_to_results = "../../KK_results/report/image_KK_lau_clu10_pooled_5_5dE1_06dE1_5iter.pkl"
...
mask_ss_conv = np.sum((np.absolute(im.ss[:,:,0,:]/ss_1)>3) * (np.absolute(im.ss[:,:,0,:]) > 200), axis = 2) >0
im.plot_heatmap((mask_ss_conv)*1, title = r"pixels with non-converging surface scattering \n in " + name, discrete_colormap = True, color_bin_size =1, save_as = save_loc + "non_conv_ss")
im.plot_heatmap(((~mask)*mask_ss_conv)*1, title = r"overlap between non-converging pixels, \n and pixels with WS$_2$ present", discrete_colormap = True, color_bin_size =1, save_as = save_loc + "overlap_non_conv_ss")
#%%
"""
...
im.plot_heatmap(im.t[:,:,0], title = "thickness where surface scattering converges in\n"+ name , cbar_kws={'label': '[nm]'}, cmap = cmap, mask = mask_ss, vmax = 200, save_as = save_loc + "t_ssconv_5iter")
im.plot_heatmap(np.absolute(im.t[:,:,0]), title = "thickness where surface scattering does not \nconverge in "+ name , cbar_kws={'label': '[nm]'}, cmap = cmap, vmax = 50, mask = ~mask_ss, save_as = save_loc + "t_not_ssconv_5iter")
im.plot_heatmap(im.ssratio[:,:,0], title = "surface scattering influence ratio "+ name , cbar_kws={'label': '[-]'}, cmap = cmap, mask = mask_ss, save_as = save_loc + "ssratio_5iter", vmax = 0.1)
...
im.plot_heatmap(E_round, title = "discretized bandgap energies "+ name + ",\n surface corrected, capped at 1.3", cbar_kws={'label':  'energy [eV]'}, cmap = cmap, vmax = 1.2, mask = (mask | mask_E_band), save_as = save_loc + "E_band_sscor_discr_capped", color_bin_size = size_E_bins, discrete_colormap = True, sig=2)
# im.plot_heatmap(E_round, title = "discretized bandgap energies "+ name, cbar_kws={'label':  'energy [eV]'}, cmap = cmap, mask = (mask | mask_E_band), vmax = 1.2, save_as = save_loc + "E_band_discr_extra", color_bin_size = size_E_bins, discrete_colormap = True, sig=2)
